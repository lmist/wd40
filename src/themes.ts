...
// Helper to build full ChromeColors from a minimal set
function darkChrome(c: {
  bg: string; bgSurface: string; bgEditor: string;
  fg: string; fgDim: string; accent: string;
  accentDim: string; border: string; statusbarBg: string;
  fgSecondary?: string; accentHover?: string; accentSecondary?: string;
  toolbarBg?: string; toolbarBorder?: string; modeBg?: string; modeFg?: string;
}): ChromeColors {
  return {
    ...c,
    fgSecondary: c.fgSecondary || c.fgDim,
    accentHover: c.accentHover || c.accentDim.replace(/[\d.]+\)$/, (m) => `${Math.min(parseFloat(m) * 1.5, 0.25).toFixed(2)})`),
    accentSecondary: c.accentSecondary || c.accent,
    toolbarBg: c.toolbarBg || c.bgSurface + "e0",
    toolbarBorder: c.toolbarBorder || c.border,
    modeBg: c.modeBg || c.accentDim,
    modeFg: c.modeFg || c.accent,
  };
}

function lightChrome(c: {
  bg: string; bgSurface: string; bgEditor: string;
  fg: string; fgDim: string; accent: string;
  accentDim: string; border: string; statusbarBg: string;
  fgSecondary?: string; accentHover?: string; accentSecondary?: string;
  ...
  return {
    ...c,
    fgSecondary: c.fgSecondary || c.fgDim,
    accentHover: c.accentHover || c.accentDim.replace(/[\d.]+\)$/, (m) => `${Math.min(parseFloat(m) * 1.8, 0.2).toFixed(2)})`),
    accentSecondary: c.accentSecondary || c.accent,
    toolbarBg: c.toolbarBg || c.bgSurface + "eb",
    toolbarBorder: c.toolbarBorder || c.border,
    modeBg: c.modeBg || c.accentDim,
    modeFg: c.modeFg || c.accent,
  };
...
export const THEMES: ThemeEntry[] = [
  // ── Custom warm theme (default) ──
  {
    ...
    extension: oneDark,
    chrome: darkChrome({
      bg: "#1a1a1c", bgSurface: "#202022", bgEditor: "#17171a",
      fg: "#e8e6e3", fgDim: "#555553", accent: "#e09050",
      accentDim: "rgba(224, 144, 80, 0.10)", border: "rgba(255, 255, 255, 0.07)",
      statusbarBg: "#17171a",
      fgSecondary: "#8a8a88", accentHover: "rgba(224, 144, 80, 0.18)",
      accentSecondary: "#6bafbd", toolbarBg: "rgba(32, 32, 34, 0.88)",
      toolbarBorder: "rgba(255, 255, 255, 0.08)",
      modeBg: "rgba(224, 144, 80, 0.10)", modeFg: "#e09050",
    }),
  ...
  },
  {
    ...
    extension: oneLight,
    chrome: lightChrome({
      bg: "#f5f4f1", bgSurface: "#f0efec", bgEditor: "#fdfcfa",
      fg: "#1a1a1a", fgDim: "#a8a8a6", accent: "#c4652a",
      accentDim: "rgba(196, 101, 42, 0.07)", border: "rgba(0, 0, 0, 0.07)",
      statusbarBg: "#eae9e5",
      fgSecondary: "#6e6e6c", accentHover: "rgba(196, 101, 42, 0.14)",
      accentSecondary: "#2878a8", toolbarBg: "rgba(250, 249, 247, 0.92)",
      toolbarBorder: "rgba(0, 0, 0, 0.06)",
      modeBg: "rgba(196, 101, 42, 0.08)", modeFg: "#c4652a",
    }),
